...
def redefine_graph(
    ...
    assert (
        sum([no_node_mpfs, no_node_chem_feats, rand_node_feats]) <= 1
    ), "Can only set one of no_node_mpfs, no_node_chem_feats, or rand_node_feats to be True"

    print(
        f"Adjusting edge index and attaching edge weights for cutoff distance {cutoff_distance}"
    )
    ...
    for sample_set in graph_dict:
        for sample in graph_dict[sample_set]:
            ...
            assert graph.dataset[0].x.size(1) == 18, "Reload graph_dict"
            ...
            edge_index, d_array = graph._get_protein_struct_edges(
                graph.nodes.center_of_mass(compound="residues")
            )
            edge_dists = graph._gen_edge_dists(edge_index, d_array)
            edge_attr = graph.calc_edge_weights(
                edge_weight_func, edge_dists, lambda_param
            )
            ...
            if shuffle_edges:
                ...
                edge_attr = graph.dataset[0].edge_attr[
                    torch.randperm(len(graph.dataset[0].edge_attr), generator=g)
                ]

                # change edge index and edge weights for Data object
                graph_dict[sample_set][sample]["graph"].dataset[
                    0
