...
def calculate_metrics(model, data_loader):
    ...
    for data in data_loader:  # Iterate in batches over the test dataset.
        ...
        out = eval_model(
            eval_data.x, eval_data.edge_index, eval_data.edge_attr, eval_data.batch
        )

        if eval_model.lin.out_features == 1:
            pred = (
                out > 0.5
            ).float()  # Convert output to binary predictions using a threshold of 0.5
        else:
    ...
    print(
        f"Number predicted resistant = {num_pred_pos}. Number labeled R = {num_true_pos}"
    )
    print(
        f"Number predicted susceptible = {num_pred_neg}. Number labeled S = {num_true_neg}"
    )
