...
def train(config, graph_dict, wandb_run_name=None):
    ...
    model = run_model.pnca_GCN_vary_graph(
        ...
        save_path=(
            f'{config.get("save_path", None)}/{wandb_run_name}'
            if wandb_run_name
            else config.get("save_path", None)
        ),
        wandb_params={
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to sweep config YAML file (required if sweep: true in config)",
    )
    parser.add_argument(
        "--count", type=int, default=None, help="Number of runs to execute in the sweep"
    )
    ...
    if config.get("sweep", False):
        ...
        if args.sweep_config is None:
            raise ValueError(
                "--sweep_config is required when sweep: true in config.yaml"
            )
